...
def profile(username):
    ...
    return render_template('user/profile.html', user=user, notes=notes, link_form=link_form, is_following=is_following)
...
def add_link():
    form = UserLinkForm()
    if form.validate_on_submit():
        link = UserLink(user_id=current_user.id, title=form.title.data, url=form.url.data)
        db.session.add(link)
        db.session.commit()
        flash('Link added to your profile.', 'success')
        log_activity(current_user.id, 'add_link', 'user_link', link.id, f'{current_user.username} added a custom link')
    else:
...
def follow(username):
    ...
    if not existing_follow:
        follow_rel = Follow(follower_id=current_user.id, following_id=user.id)
        db.session.add(follow_rel)
        
        # Create notification for the followed user
        notif = Notification(user_id=user.id, message=f'{current_user.username} started following you.')
        db.session.add(notif)
        
        db.session.commit()
        
        # Real-time notification ping
        socketio.emit('new_notification', {'count_increment': 1}, room=f"user_{user.id}")
        
        log_activity(current_user.id, 'follow', 'user', user.id, f'{current_user.username} followed {user.username}')
...
def unfollow(username):
    ...
    if existing_follow:
        ...
        db.session.commit()
        log_activity(current_user.id, 'unfollow', 'user', user.id, f'{current_user.username} unfollowed {user.username}')
